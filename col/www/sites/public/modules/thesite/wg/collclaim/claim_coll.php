...
// 20190409 cooly
function _build_online_claiming_table() {
  $schema = array(
    'description' => 'online claiming table',
    'fields' => array(
      'cid' => array('type' => 'int', 'unsigned' => TRUE, 'not null' => TRUE),
    ),
    'primary key' => array('cid')
  );
  _create_table(ONLINE_CLAIM_COLL, $schema);
}


function _collclaim_add_onlineclaim($claim_id) {
  db_set_active(CLAIM_DB); // Use CLAIM database
  if(!db_table_exists(ONLINE_CLAIM_COLL)) _build_online_claiming_table();
  ...
  try {
    $query = db_insert(ONLINE_CLAIM_COLL)->fields($obj);
    $id = $query->execute();
  } catch(Exception $e) {
...
function _build_claiming_table() {
  $schema = array(
    ...
    'fields' => array(
      ...
      'online'    => array('type' => 'int', 'not null' => TRUE, 'default' => 0)                        // 0: paper, 1: online
    ),
...
function _collclaim_add_claim($uid, $collection_id, $hd, $copyright, $display, $note, $openmosaic, $online = 0) {
  db_set_active(CLAIM_DB); // Use CLAIM database
...
function _collclaim_get_claims_of_an_author($user_id) {
  ...
  $query->fields('c', array('id', 'cid', 'uid', 'hd', 'copyright', 'display', 'note', 'created', 'verified', 'openmosaic', 'online')) //SELECT the fields from CLAIM_COLL
        ->orderBy('id', 'ASC');
        //->orderBy('created', 'DESC'); //ORDER BY created
        //->range(0,2); //LIMIT to 2 records
...
function get_verified_claims_of_a_collection($cid) {
  ...
  $query->fields('c', array('id', 'cid','open', 'note', 'created', 'verified', 'openmosaic', 'online')) //SELECT the fields from CLAIM_COLL
